...
import SwapCostEstimator from '@/components/tools/trading/SwapCostEstimator'

export const metadata: Metadata = {
  title: 'Swap Cost Estimator | FincTools',
  description: 'Estimasi biaya swap dan rollover untuk posisi overnight berdasarkan lot size dan swap rate.',
}
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        <nav className="flex items-center gap-1 text-xs text-[--text-secondary] mb-6">
          ...
          <ChevronRight size={12} />
          <span className="text-[--text-primary] font-medium">Swap Cost Estimator</span>
        </nav>
        <div className="mb-8">
          ...
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-[--text-primary] mb-2">Swap Cost Estimator</h1>
          <p className="text-[--text-secondary] leading-relaxed max-w-xl">Estimasi biaya swap dan rollover untuk posisi overnight berdasarkan lot size dan swap rate.</p>
        </div>
        ...
        <div className="finc-card bg-slate-50 dark:bg-slate-900/50">
          ...
          <div className="flex flex-wrap gap-2">
            {related.filter(t => !t.href.includes('swap-cost-estimator')).slice(0,4).map(t => (
