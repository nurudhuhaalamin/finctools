...
import ProbabilityOfRuinAnalyzer from '@/components/tools/trading/ProbabilityOfRuinAnalyzer'

export const metadata: Metadata = {
  title: 'Probability of Ruin Analyzer | FincTools',
  description: 'Kalkulasi probabilitas bangkrut berdasarkan win rate, R/R ratio, dan risiko per trade.',
}
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        <nav className="flex items-center gap-1 text-xs text-[--text-secondary] mb-6">
          ...
          <ChevronRight size={12} />
          <span className="text-[--text-primary] font-medium">Probability of Ruin Analyzer</span>
        </nav>
        <div className="mb-8">
          ...
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-[--text-primary] mb-2">Probability of Ruin Analyzer</h1>
          <p className="text-[--text-secondary] leading-relaxed max-w-xl">Kalkulasi probabilitas bangkrut berdasarkan win rate, R/R ratio, dan risiko per trade.</p>
        </div>
        <div className="finc-card mb-8">
          <ProbabilityOfRuinAnalyzer />
        </div>
        <div className="finc-card bg-slate-50 dark:bg-slate-900/50">
          ...
          <div className="flex flex-wrap gap-2">
            {related.filter(t => !t.href.includes('probability-of-ruin-analyzer')).slice(0,4).map(t => (
