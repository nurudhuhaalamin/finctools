...
import StreakAnalyzer from '@/components/tools/trading/StreakAnalyzer'

export const metadata: Metadata = {
  title: 'Streak Analyzer | FincTools',
  description: 'Hitung probabilitas terjadinya serial win atau loss beruntun berdasarkan win rate historis.',
}
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        <nav className="flex items-center gap-1 text-xs text-[--text-secondary] mb-6">
          ...
          <ChevronRight size={12} />
          <span className="text-[--text-primary] font-medium">Streak Analyzer</span>
        </nav>
        <div className="mb-8">
          ...
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-[--text-primary] mb-2">Streak Analyzer</h1>
          <p className="text-[--text-secondary] leading-relaxed max-w-xl">Hitung probabilitas terjadinya serial win atau loss beruntun berdasarkan win rate historis.</p>
        </div>
        ...
        <div className="finc-card bg-slate-50 dark:bg-slate-900/50">
          ...
          <div className="flex flex-wrap gap-2">
            {related.filter(t => !t.href.includes('streak-analyzer')).slice(0,4).map(t => (
