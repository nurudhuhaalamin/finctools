...
export const metadata: Metadata = {
  title: 'Stop Loss Optimizer | FincTools',
  description: 'Tentukan level stop loss optimal berdasarkan ATR dan volatilitas untuk perlindungan modal maksimal.',
}
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        <nav className="flex items-center gap-1 text-xs text-[--text-secondary] mb-6">
          ...
          <ChevronRight size={12} />
          <span className="text-[--text-primary] font-medium">Stop Loss Optimizer</span>
        </nav>
        <div className="mb-8">
          ...
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-[--text-primary] mb-2">Stop Loss Optimizer</h1>
          <p className="text-[--text-secondary] leading-relaxed max-w-xl">Tentukan level stop loss optimal berdasarkan ATR dan volatilitas untuk perlindungan modal maksimal.</p>
        </div>
        ...
        <div className="finc-card bg-slate-50 dark:bg-slate-900/50">
          ...
          <div className="flex flex-wrap gap-2">
            {related.filter(t => !t.href.includes('stop-loss-optimizer')).slice(0,4).map(t => (
