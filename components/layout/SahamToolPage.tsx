...
export interface SahamToolPageProps {
  name: string
  subcategory?: string
  description: string
...
export default function SahamToolPage({
  name, subcategory = 'Strategi Beli', description,
  ...
  return (
    <>
      <div className="mb-8">
        <div className="inline-flex items-center text-xs font-medium text-indigo-600
                        dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/30
          ...
          {variables.map(v => (
            <div key={v.name} className="flex gap-3 items-start">
              <code className="text-indigo-600 dark:text-indigo-400 font-mono text-xs
                               bg-indigo-50 dark:bg-indigo-950/30 px-2 py-0.5
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody className="divide-y divide-[--border]">
                {interpretation.map(row => (
                  <tr key={row.range}>
                    <td className="py-2.5 pr-4 font-mono text-xs text-indigo-600 dark:text-indigo-400">{row.range}</td>
          ...
          {faqs.map((f, i) => (
            <details key={i} className="finc-card group">
              <summary className="cursor-pointer text-sm font-semibold text-[--text-primary]
                ...
                {f.q}
                <span className="text-indigo-500 shrink-0 text-lg leading-none
