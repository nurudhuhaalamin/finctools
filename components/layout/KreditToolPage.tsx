...
export interface KreditToolPageProps {
  name: string
  subcategory?: string
  description: string
...
export default function KreditToolPage({
  name, subcategory = 'Kredit & Properti', description,
  ...
  return (
    <>
      <div className="mb-8">
        <div className="inline-flex items-center text-xs font-medium text-orange-600
                        dark:text-orange-400 bg-orange-50 dark:bg-orange-950/30
                        border border-orange-100 dark:border-orange-900
                        rounded-full px-3 py-1 mb-3">
          Kredit & Properti · {subcategory}
        </div>
        ...
        <div className="space-y-3">
          {steps.map((s, i) => (
            <div key={i} className="flex gap-4 finc-card py-3">
              <div className="w-7 h-7 rounded-full bg-orange-100 dark:bg-orange-900/30
        ...
        <div className="space-y-2">
          {variables.map(v => (
            <div key={v.name} className="flex gap-3 items-start">
              <code className="text-orange-600 dark:text-orange-400 font-mono text-xs
                               bg-orange-50 dark:bg-orange-950/30 px-2 py-0.5
      ...
      {interpretation && interpretation.length > 0 && (
        <section className="mb-10">
          <h2 className="font-heading text-xl font-bold text-[--text-primary] mb-4">
            {interpretationTitle || 'Panduan Interpretasi'}
          </h2>
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody className="divide-y divide-[--border]">
                {interpretation.map(row => (
                  <tr key={row.range}>
                    <td className="py-2.5 pr-4 font-mono text-xs text-orange-600 dark:text-orange-400">{row.range}</td>
          ...
          {faqs.map((f, i) => (
            <details key={i} className="finc-card group">
              <summary className="cursor-pointer text-sm font-semibold text-[--text-primary]
                ...
                {f.q}
                <span className="text-orange-500 shrink-0 text-lg leading-none
