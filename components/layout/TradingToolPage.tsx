...
export interface TradingToolPageProps {
  /* Header */
  name: string
  category?: string
  description: string
    ...
    <>
      ...
      <section className="mb-10">
        ...
        <div className="finc-card bg-navy-900 dark:bg-navy-950 text-white mb-4">
          ...
          <p className="font-mono text-finc-green text-sm leading-relaxed whitespace-pre-line">
            {formula}
          </p>
          {formulaExample && (
            <p className="font-mono text-slate-300 text-xs mt-3 leading-relaxed whitespace-pre-line">
              {formulaExample}
            </p>
          )}
        </div>
        <div className="space-y-2">
          {variables.map(v => (
            <div key={v.name} className="flex gap-3 items-start">
              <code className="text-finc-green font-mono text-xs bg-finc-green/10
                               px-2 py-0.5 rounded shrink-0 h-fit mt-0.5">
                {v.name}
              </code>
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody className="divide-y divide-[--border]">
                {interpretation.map(row => (
                  <tr key={row.range}>
                    <td className="py-2.5 pr-4 font-mono text-xs text-finc-green">{row.range}</td>
